...
// $(function () {
// 	let $banner = $('.banner'),
// 		$slidesList = $banner.find('.slides'),
// 		$slickoDotsList = $banner.find('.slick-dots>li'),
// 		$slickPrev  = $banner.children('.slick-prev '),
// 		$slickNext = $banner.children('.slick-next');
// 	let step = 0,
// 		prev = 0,
...
// 	// CHANGE实现切换
// 	function change() {
// 		let $cur = $slidesList.eq(step),
// 			$pre = $slidesList.eq(prev);
// 		$cur.css('zIndex', 1);
// 		$pre.css('zIndex', 0);
...
// 		// $paginationList.eq(step).addClass('active')
// 		// 	.siblings().removeClass('active');
// 		$slickoDotsList.each((index, item) => {
// 			if (index === step) {
...
// 	// // 鼠标进入和离开控制自动的关闭和开启
// 	// $slickoDotsList.on('mouseenter', () => clearInterval(autoTimer))
// 	// 	.on('mouseleave', () => autoTimer = setInterval(autoMove, interval));

// 	// 点击焦点切换
// 	$slickoDotsList.on('click', function () {
// 		//  $(this).index() 当前点击这一项的索引
// 		let index = $(this).index();
// 		if (index === step) return;
// 		prev = step;
